...
export const columns = [
  {
    ...
    "width": 150,
      ...
      formatter: ({row, otherFunctions}) => {
        ...
        if ((row.LEVEL_FLAG === 1 && row.IS_THREE_LEVEL === 'Y') || row.LEVEL_FLAG === 0) {
          return (
...
export const formSaveData = (data, udfcolumns, moduleId) => {
  const saveData = data.map(d => {
    ...
    return ({
      "FUNDING_ID": d.FUNDING_ID || null,
      "PARENT_FUNDING_ID": d.PARENT_FUNDING_ID || null,
      ...
      ...(d.LEVEL_FLAG === 2 ? { "FUND_UDF_DATA": udfData} : {}),
    })
  })
  return saveData
}
