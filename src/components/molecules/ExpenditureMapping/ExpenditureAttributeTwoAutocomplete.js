...
export default function ExpenditureAttributeTwoAutocomplete({
  row,
  column,
  onRowChange,
  onClose,
  ...
  const [input, setInput] = useState(row?.EXPENDITURE_ATTRIBUTE_2 || '')
    ...
    const callApi = async () => {
      ...
      try {
        const response = await apiCall({
          method: 'get',
          url: '/GetExpenditureAttribute2List',
          params: {
            Active: 'y',
  ...
  const onChange = (newValue) => {
    ...
    onRowChange({ ...row, EXPENDITURE_ATTRIBUTE_2: val?.DESCRIPTION, EXPENDITURE_ATTRIBUTE_2_ID: val?.ID })
    setInput(val?.DESCRIPTION)
    const { getChange } = otherFunctions
    if (getChange) {
      getChange({ ...row, EXPENDITURE_ATTRIBUTE_2: val?.DESCRIPTION, EXPENDITURE_ATTRIBUTE_2_ID: val?.ID }, column.key)
    }
  }
  const onBlur = () => {
    onClose(true)
  ...
  return (
    <Autocomplete
      ...
      loading={loading}
      value={row.EXPENDITURE_ATTRIBUTE_2 || ''}
      isOptionEqualToValue={(option, value) => option.DESCRIPTION === value.DESCRIPTION}
      id="ExpenditureAttributeTwoAutocomplete"
      onBlur={onBlur}
      freeSolo
      fullWidth
