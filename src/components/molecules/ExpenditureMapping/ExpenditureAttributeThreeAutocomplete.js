...
export default function ExpenditureAttributeThreeAutocomplete({
  row,
  column,
  onRowChange,
  onClose,
  ...
  const [input, setInput] = useState(row?.EXPENDITURE_ATTRIBUTE_3 || '')
  ...
  const onChange = (newValue) => {
    ...
    onRowChange({ ...row, EXPENDITURE_ATTRIBUTE_3: val?.DESCRIPTION, EXPENDITURE_ATTRIBUTE_3_ID: val?.ID })
    setInput(val?.DESCRIPTION)
    const { getChange } = otherFunctions
    if (getChange) {
      getChange({ ...row, EXPENDITURE_ATTRIBUTE_3: val?.DESCRIPTION, EXPENDITURE_ATTRIBUTE_3_ID: val?.ID }, column.key)
    }
  }
  const onBlur = () => {
    onClose(true)
  ...
  return (
    <Autocomplete
      ...
      loading={loading}
      value={row.EXPENDITURE_ATTRIBUTE_3 || ''}
      isOptionEqualToValue={(option, value) => option.DESCRIPTION === value.DESCRIPTION}
      id="ExpenditureAttributeThreeAutocomplete"
      onBlur={onBlur}
      freeSolo
      fullWidth
