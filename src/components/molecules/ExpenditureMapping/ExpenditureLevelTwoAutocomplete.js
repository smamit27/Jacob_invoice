...
export default function ExpenditureLevelTwoAutocomplete({
  row,
  column,
  onRowChange,
  onClose,
  ...
  const [input, setInput] = useState(row?.CLIENT_EXPENDITURE_LEVEL_2 || '')
  ...
  const onChange = (newValue) => {
    ...
    onRowChange({ ...row, CLIENT_EXPENDITURE_LEVEL_2: val?.DESCRIPTION, CLIENT_EXPENDITURE_LEVEL_2_ID: val?.ID })
    setInput(val?.DESCRIPTION)
    const { getChange } = otherFunctions
    if (getChange) {
      getChange({ ...row, CLIENT_EXPENDITURE_LEVEL_2: val?.DESCRIPTION, CLIENT_EXPENDITURE_LEVEL_2_ID: val?.ID }, column.key)
    }
  }
  const onBlur = () => {
    onClose(true)
  ...
  return (
    <Autocomplete
      ...
      id="ExpenditureLevelTwoAutocomplete"
      onBlur={onBlur}
      freeSolo
      fullWidth
