...
export default function ExpenditureLevelThreeAutocomplete({
  row,
  column,
  onRowChange,
  onClose,
  ...
  const [input, setInput] = useState(row?.CLIENT_EXPENDITURE_LEVEL_3 || '')
  ...
  const onChange = (newValue) => {
    ...
    onRowChange({ ...row, CLIENT_EXPENDITURE_LEVEL_3: val?.DESCRIPTION, CLIENT_EXPENDITURE_LEVEL_3_ID: val?.ID })
    setInput(val?.DESCRIPTION)
    const { getChange } = otherFunctions
    if (getChange) {
      getChange({ ...row, CLIENT_EXPENDITURE_LEVEL_3: val?.DESCRIPTION, CLIENT_EXPENDITURE_LEVEL_3_ID: val?.ID }, column.key)
    }
  }
  const onBlur = () => {
    onClose(true)
  ...
  return (
    <Autocomplete
      ...
      loading={loading}
      value={row.CLIENT_EXPENDITURE_LEVEL_3 || ''}
      isOptionEqualToValue={(option, value) => option.DESCRIPTION === value.DESCRIPTION}
      id="ExpenditureLevelThreeAutocomplete"
      onBlur={onBlur}
      freeSolo
      fullWidth
