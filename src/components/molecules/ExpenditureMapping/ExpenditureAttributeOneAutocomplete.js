...
export default function ExpenditureLevelOneAutocomplete({
  ...
  const [input, setInput] = useState(row?.EXPENDITURE_ATTRIBUTE_1 || '')
    ...
    const callApi = async () => {
      ...
      try {
        const response = await apiCall({
          method: 'get',
          url: '/GetExpenditureAttribute1List',
          params: {
            Active: 'y',
  ...
  const onChange = (newValue) => {
    ...
    onRowChange({ ...row, EXPENDITURE_ATTRIBUTE_1: val?.DESCRIPTION, EXPENDITURE_ATTRIBUTE_1_ID: val?.ID })
    setInput(val?.DESCRIPTION)
    const { getChange } = otherFunctions
    if (getChange) {
      getChange({ ...row, EXPENDITURE_ATTRIBUTE_1: val?.DESCRIPTION, EXPENDITURE_ATTRIBUTE_1_ID: val?.ID }, column.key)
    }
  }
  const onBlur = () => {
    onClose(true)
  ...
  return (
    <Autocomplete
      ...
      loading={loading}
      value={row.EXPENDITURE_ATTRIBUTE_1 || ''}
