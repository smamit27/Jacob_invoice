...
export default function ExpenditureLevelOneAutocomplete({
  row,
  column,
  onRowChange,
  onClose,
  ...
  const [input, setInput] = useState(row?.CLIENT_EXPENDITURE_LEVEL_1 || '')
  ...
  const onChange = (newValue) => {
    ...
    onRowChange({ ...row, CLIENT_EXPENDITURE_LEVEL_1: val?.DESCRIPTION, CLIENT_EXPENDITURE_LEVEL_1_ID: val?.ID })
    setInput(val?.DESCRIPTION)
    const { getChange } = otherFunctions
    if (getChange) {
      getChange({ ...row, CLIENT_EXPENDITURE_LEVEL_1: val?.DESCRIPTION, CLIENT_EXPENDITURE_LEVEL_1_ID: val?.ID }, column.key)
    }
  }
  const onBlur = () => {
    onClose(true)
  ...
  return (
    <Autocomplete
      ...
      loading={loading}
      value={row.CLIENT_EXPENDITURE_LEVEL_1 || ''}
      isOptionEqualToValue={(option, value) => option.DESCRIPTION === value.DESCRIPTION}
      id="ExpenditureLevelOneAutocomplete"
      onBlur={onBlur}
      freeSolo
      fullWidth
