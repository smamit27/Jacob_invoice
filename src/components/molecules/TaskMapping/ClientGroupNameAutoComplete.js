...
export default function ClientGroupNameAutoComplete({
  row,
  column,
  onRowChange,
  onClose,
  ...
  const [input, setInput] = useState(row?.CLIENT_TASK_GROUP_NAME || '')
  ...
  const onChange = (val) => {
    onRowChange({ ...row, CLIENT_TASK_GROUP_NAME: val?.Description, CLIENT_TASK_GROUP_ID: val?.ID })
    setInput(val?.Description)
    const { getChange } = otherFunctions
    if (getChange) {
      getChange({ ...row, CLIENT_TASK_GROUP_NAME: val?.Description, CLIENT_TASK_GROUP_ID: val?.ID,GROUP:'Y' }, column.key)
    }
  }
  const onBlur = () => {
    onClose(true)
  ...
  return (
    <Autocomplete
      ...
      loading={loading}
      value={row.CLIENT_TASK_GROUP_NAME || ''}
      isOptionEqualToValue={(option, value) => option.Description === value.Description}
      id="ClientGroupNameAutoComplete"
      onBlur={onBlur}
      freeSolo
      fullWidth
