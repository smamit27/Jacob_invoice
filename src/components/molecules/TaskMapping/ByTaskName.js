...
import {saveTaskNumberNameAction,saveTaskNumberNameResetAction} from './logic'
import { Icon } from '../../atoms';
import {formSaveDataNameTaskMapping} from  './constants'

function ByTaskName({ open = false, onClose = () => null }) {
  ...
  const { error:saveError, flag:saveFlag } = useSelector(state=>state.saveTaskNumberNameReducer)

  useEffect(() => {
    if (saveFlag) {
      dispatch(saveTaskNumberNameResetAction());
  ...
  const onSave = () => {
    ...
    const payload = {
      data: formSaveDataNameTaskMapping(selectedRows?.current?.filter(d => d.IS_OVERRIDE),'Y'),
      params: request
    }     
  ...
  const onSubmit = (task) => {
    ...
    const setProjectList = getTaskMappingProjectList.map(item => {
      ...
      if (task.TASK_NUMBER === "" && (item.LONG_TASK_NAME?.toLowerCase()?.includes(task.LONG_TASK_NAME?.toLowerCase()) && task.LONG_TASK_NAME !== "")) {
        selectedRow.TASK_NUMBER_OVERRIDE = task.TASK_MAP_TO;
  ...
  return (
    <Modal onClose={onClose} open={open} >
      <Box sx={style} className="quickTaskcharModal">
        <Header >
          <HeaderTitle data-testid="quick-task-mapping-number-name">
            Quick Task Mapping - By Number/Name Contains
          </HeaderTitle>
          <div onClick={onClose}>
            <Icon name="close" />
          </div>
        ...
        <Grid container >
          ...
          <Grid item xs={12} lg={4} p={3} className={classes.borderListCollection}>
            <form onSubmit={handleSubmit(onSubmit)} className="taskNumberForm">
              <Grid item xs={12} mt={2} className="taskNumberInputForm">
              ...
              <Grid item xs={12} mt={2}>
                <Button type="submit" variant="contained" disabled={dissabledMapTo} >
                  Map Tasks
                </Button>
                <Button style={{textDecoration: "underline" }} onClick={clearAll} className="clearButton">Clear All</Button>

              </Grid>
            </form>
          </Grid>
          <Grid item xs={12} lg={8} p={3}>
            <Grid item xs={12} className="previousMapContainer">
              <Label>
                  ...
                  <Table aria-label="customized table">
                    <TableHead>
                      <TableRow>
                        ...
                        <StyledTableCell>Task Number Mapped</StyledTableCell>
                    ...
                    <TableBody>
                      {rowsData.map((row) => (
                        <StyledTableRow key={row.TASK_ID}>
                          ...
                          <StyledTableCell align="left">{row.LONG_TASK_NAME}</StyledTableCell>
...
export default memo(ByTaskName);
