...
import { Modal, Box, styled, Button, Stack, Grid, TextField, IconButton, Checkbox } from '@mui/material'
...
function CopyEntries({ open, onClose, modalData = { selected: [], all: [] }, onCopyApply }) {
  ...
  function onPreview () {
    ...
    if (!isOverlap?.length) {
      const newRows = rowsData.map(d => ({
        ...
        END_DATE: end ? format(end, 'dd-MMM-yyyy') : '',
  ...
  const onApply = () => {
    Promise.all(rows.map(() => getUniqueIdFromApi('/GetBillingSeq', 'BTR_ID'))).then((values) => {
      const newRows = rows.map(({ 
        tableRowId, parentRowId, ROW_NUM, FLAG_IS, BTR_ID1, MINIMUM_RATE_PERCENT, CAPPED_RATE_PERCENT, CAPPED_RATE_OLD, MINIMUM_RATE_OLD, ...Others
      }, i) => ({
        ...Others,
        "ADDED_DATE": null,
        "ADDED_BY": null,
  ...
  return (
    <Modal open={open}>
      <Box sx={style}>
        <Header>
          <ModalTitle data-testid='titles-rates-copy-entries'>Copy Entries</ModalTitle>
          ...
          <Grid container spacing={2} >
            ...
            <Grid item sm={12} xs={12} md={9} lg={9}>
              <Stack spacing={2} >
                ...
                <DataGrid className='rdg-header-white' rowKeyGetter={rowKeyGetter} rowHeight={40} headerRowHeight={60} columns={columns} rows={rows} style={{height: 'calc(80vh*(9/13))' }} />
              </Stack>
            </Grid>
          </Grid>
        </FormContent>
