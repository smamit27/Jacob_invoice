...
export default function SubcontractorBTAutocomplete({
  row,
  column,
  onRowChange,
  onClose,
  ...
  const [input, setInput] = useState(row?.SUBCONTRACTOR_BILLING_TITLE_NAME || '')
    ...
    const callApi = async () => {
      ...
      try {
        const response = await apiCall({
          method: 'get',
          url: '/GetSubContractorBillingTitle',
          params: {
  ...
  const onChange = (newValue) => {
    ...
    onRowChange({ ...row, SUBCONTRACTOR_BILLING_TITLE_NAME: val?.DESCRIPTION, SUBCONTRACTOR_BILLING_TITLE: val?.ID })
    setInput(val?.DESCRIPTION)
    const { getChange } = otherFunctions
    if (getChange) {
      getChange({ ...row, SUBCONTRACTOR_BILLING_TITLE_NAME: val?.DESCRIPTION, SUBCONTRACTOR_BILLING_TITLE: val?.ID }, column.key)
    }
  }
  const onBlur = () => {
    onClose(true)
  ...
  return (
    <Autocomplete
      ...
      loading={loading}
      value={row.SUBCONTRACTOR_BILLING_TITLE_NAME || ''}
