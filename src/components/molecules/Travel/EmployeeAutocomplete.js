...
export default function EmployeeAutocomplete({
  ...
  const [input, setInput] = useState(row?.EMPLOYEE_NUMBER || '')
  ...
  const onChange = (val) => {
    onRowChange({ ...row, EMPLOYEE_NUMBER: val?.EMPLOYEE_NUMBER, EMPLOYEE_ID: val?.EMPLOYEE_ID, EMPLOYEE_NAME: val?.EMPLOYEE_NAME })
    setInput(val?.EMPLOYEE_NUMBER)
    const { getChange } = otherFunctions
    if (getChange) {
      getChange({ ...row, EMPLOYEE_NUMBER: val?.EMPLOYEE_NUMBER, EMPLOYEE_ID: val?.EMPLOYEE_ID, EMPLOYEE_NAME: val?.EMPLOYEE_NAME }, column.key)
    }
  }
  const onBlur = () => {
  ...
  return (
    <Autocomplete
      ...
      getOptionLabel={option => {
        return (typeof option === 'string' ? option : option?.EMPLOYEE_NUMBER) || ''
      }}
      loading={loading}
      value={row.EMPLOYEE_NUMBER || ''}
      isOptionEqualToValue={(option, value) => option.EMPLOYEE_NUMBER === value.EMPLOYEE_NUMBER}
