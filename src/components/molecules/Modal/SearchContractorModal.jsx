...
import { Button, Grid, Stack, Box, DialogActions,DialogContent,DialogTitle} from '@mui/material'
...
import { contractAdministrator } from '../../../redux/common/action'
...
function SearchContractorModal(props) {
  const {setOpen} = props;
  ...
  const handleSearchBtn = () => {
    ...
    for (let key in selectedEmployee) {
      if (selectedEmployee[key] !== "")
        dispatch(getEmployeesDetails(selectedEmployee,'contractor'))
        setEmpList(true);  
    }  
    return false;
  }
  const handleEmployeeList = (emp) => {
    ...
    setSearchData(true)
    dispatch(contractAdministrator(emp))
  ...
  const handleClose =() =>{
    ...
    return (
        ...
        <Box display="flex" flexDirection="row" justifyContent="space-between">
          <Label  variant="body1" pt={1} pl={1} style={{ color: "#222222",fontSize:"14px",fontWeight: "400",fontFamily: "Jacobs Chronos bold",paddingLeft:'10px' }} 
                      data-testid="select-contract-administrator">Select Contract Administrator</Label>
...
<>
        ...
        <Grid  item xs={12}>
          <BootstrapButton style={{paddingLeft:'4px'}} onClick={toggleSearchbtn}>{toggleSearch ? 'Basic Search': 'Advanced Search'}</BootstrapButton>
        </Grid>
